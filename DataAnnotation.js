...
const { JSDOM } = jsdom;*/
const urlDoc = 'https://docs.google.com/document/d/e/2PACX-1vTER-wL5E8YC9pxDx43gk8eIds59GtUUk4nJo_ZWagbnrH0NFvMXIw6VWFLpf5tWTZIT9P9oLIoFJ6A/pub'
...
function getHtmlData(url){
    fetch(url).then(response => {
        ...
        const allValuesArr = Array.from(docu.getElementsByClassName('c0')).map((text)=> text.textContent);
        ...
        const graphicDisplay = coordShapesObjArr.map((obj, index)=>{
            const uniCode1 = 9608;
            const uniCode2 = 9617;
            const blockChar = String.fromCharCode(uniCode1);
            const shadeChar = String.fromCharCode(uniCode2);
            const color = coordShapesObjArr.character === blockChar? 'black': 'grey';
            return(
                `<rect x="${obj.x}" y="${obj.y}" width="1" height="2" fill=${color}/>`
            )
